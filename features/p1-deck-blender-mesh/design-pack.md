...
`DESIGN PACK v2 | feature p1-deck-blender-mesh | design doc v1.19 | goals: G1..G7 | mechanics: M1..M5`

*v2 (2026-07-31, §3.5a Director-feedback cycle — "main-spine hallway ceiling still blocks view" post-M5/interstitial retune): adds **R-BM9** (normative cutaway overhead set — VD §7 rule 6 application to GLB-carried geometry) and **R-BM10** (volumetrics export-excluded), amends acceptance B3 with the mid–main-spine bar (B3a–B3c). No design-doc or VD bump — existing rules, clarified application to a new asset; goals/mechanics unchanged.*
...
- **R-BM9 — Normative cutaway overhead set (G2/M2; VD §7 rule 6 applied to GLB-carried geometry; v2).** For every room R, the cutaway-bound set is **not** only the `ceiling:<R>` slab: it is **all geometry whose XZ footprint intersects R's footprint and whose vertical extent lies entirely above the head-clearance datum z = 2.2 m** (VD §7 rule 1 head-height language — above every actor, silhouette, and telegraph). For main-spine this normatively includes: `ceiling:main-spine`, the deckhead/interstitial span over the corridor, spine light fixtures (z≈2.9), spine pipe runs (z≈2.3–2.7), and any deckhead dressing (hatches/plating/sensor dome) overlapping the corridor footprint. The entire set rides the room's existing fade state: **active → hidden (alpha 0); adjacent/frustum → `CEILING_FADE_ALPHA` (0.25–0.4); outside frustum → opaque**. Fade bands, adjacency logic, pop-free transitions, and the wall-top read are unchanged (VD §7 rule 6; Q5). **Realization is export-side:** the exporter joins each room's overhead set into its single `ceiling:<R>` object (multi-material join preserves material slots/primitives) and **segments the deckhead** — per-room segments join `ceiling:<R>`; only the true interstitial remainder (between-room spans) ships as `hull-interstitial:ceiling` under the existing boot binding. Engine binding, fade math, and `updateCeilingCutaway` are **untouched** (R-BM2 holds strictly). Exterior pieces overlapping no room footprint (perimeter seam ring, mast, vent stacks over envelope walls) stay opaque — VD §7 rule 9 top-band read preserved.
- **R-BM10 — Volumetrics are export-excluded (v2).** `spine_haze` (and any future volume-shader object) does **not** ship in the GLB. Three grounds: (a) Principled Volume is not glTF-representable — VD §8 standard-PBR bar; (b) atmosphere is engine-owned (Q7: lighting authority 100 % engine; VD §4.2 corridor haze is an engine lighting-matrix effect, not geometry); (c) anti-assumption — volumetric haze is unlisted in M1–M5, so it does not exist for this feature, and under the M5 all-mesh PBR retune a volume box becomes an **opaque room-filling solid** (the observed mid-spine occlusion). If corridor haze is later wanted in-engine, it is an engine atmosphere task under a future pack, never GLB cargo.
...
2. **Export set definition** — export collections: `STRUCTURE`, `ENVELOPE`, `CEILINGS`, `DRESSING`, `EXTERIOR`, `PROPS`, `TRIM`. Excluded: `SIGNAGE` (R-BM3), `LIGHTS`, `CAMERAS`, world (R-BM7), **all volume-shader objects** (R-BM10 — `spine_haze` is dropped at export).
2a. **Overhead-set join (R-BM9, v2)** — per room, join all geometry above z = 2.2 m within the room footprint (ceiling slab, ceiling-mounted fixtures, pipe runs, deckhead segment, deckhead dressing) into the single `ceiling:<R>` object, preserving material slots; segment `deckhead` so per-room spans join their `ceiling:<R>` and only true interstitial spans keep `hull-interstitial:ceiling`.
...
| B3a — **Mid–main-spine stand (v2, R-BM9/R-BM10):** player mid-corridor, spine active, AL0 **and** AL1 — corridor interior readable floor-to-wall-top (floor stripe, wall panels, doors, signage, actors/telegraphs); **zero opaque or semi-opaque overhead pixel inside the spine footprint** between camera and gameplay plane; wall tops read cleanly | G2, M2 — the Director's failing view, resolved |
| B3b — Adjacent-room doorway framing from mid-spine incl. the adjacent room's overhead set at 0.25–0.4; actors beneath readable (VD §7 rules 2/4) | G2 — ambush telegraph holds on the GLB path |
| B3c — Rooms outside the frustum keep opaque ceilings + deckhead dressing (one-vessel exterior read) | G1, VD §7 rule 9 — R-BM9 must not regress the envelope read |
