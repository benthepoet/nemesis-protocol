...
| 2026-07-30 | Director iteration 1 (§3.5a) | **Feedback (post-Gate-2, master shipped):** "We need to see bullet projectiles and I need some kind of line indicating where I'm firing." **Kimi diagnosis:** design-pack gap — presentation under-specified (v1 deferred "tracer presentation" to Grok's discretion; no visible-slug or aim-telegraph requirement existed); sim and accepted G1–G9 are correct, no design-doc rule change. **Rulings:** R7 (visible projectile tracers — emissive slug, white core / allied `#69f0ae`, ≈1.2 m along velocity, render-only), R8 (persistent in-world aim line, muzzle → first wall obstruction, dim at rest / bright while firing, off when dead). **Doc-first:** visual direction v1.3 → **v1.4** (§6 P1 row + §7 rule 7); design pack v1 → **v2** (G10, M10). **Routing:** design-layer iteration → Grok hotfix spec (Stage 2, reduced ceremony) → Composer on `feat/p1-combat-readability` → Gates 1–2. Fresh review budget per §3.5a. |
| | Escalations | none |
