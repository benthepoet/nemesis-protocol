...
describe('player determinism (G7)', () => {
  it('E14: identical replays → same pose and hash', async () => {
    ...
    const a = replay(harness.state, cmds, { current: harness.collisionWorld }, harness.graph);
    const b = replay(harness.state, cmds, { current: harness.collisionWorld }, harness.graph);
  ...
  it('E15: different streams → different hash', async () => {
    const harness = createPlayerTestHarness();
    const a = replay(harness.state, moveStream(10), { current: harness.collisionWorld }, harness.graph);
    const b = replay(harness.state, moveStream(20), { current: harness.collisionWorld }, harness.graph);
