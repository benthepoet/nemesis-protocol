...
add("""
<g id="title">
  <text x="40" y="48" fill="#e6edf3" font-size="24" font-weight="700" letter-spacing="2">ISV NEMESIS — DECK 03 // BOARDING SCHEMATIC</text>
  <text x="40" y="70" fill="#7d8fa3" font-size="12" letter-spacing="1.5">DEMON-CLASS LIGHT CRUISER &#8226; INTERNAL PLAN &#8226; PROCGEN REFERENCE v2.2 — WALL CLASSES + ANDROID RACKS</text>
...
# android rack (engineering)
for i in range(3):
    aft.append(f'<rect x="{312+i*9}" y="368" width="7" height="22" rx="3" fill="#241a20" stroke="#e040fb" stroke-width="1.2"/>')
    aft.append(f'<circle cx="{315.5+i*9}" cy="373" r="1.6" fill="#e040fb"/>')
aft.append('<text x="327" y="404" fill="#e040fb" font-size="7" text-anchor="middle" font-weight="700">ANDROID</text>')
aft.append('<text x="327" y="412" fill="#e040fb" font-size="7" text-anchor="middle" font-weight="700">RACK</text>')
...
mid.append('<line x1="345" y1="400" x2="795" y2="400" stroke="#8b949e" stroke-width="1" stroke-dasharray="14 10" opacity="0.5"/>')
mid.append('<text x="570" y="404" fill="#c9d1d9" font-size="11.5" text-anchor="middle" letter-spacing="3">MAIN SPINE</text>')
...
for r_ in range(2):
    mid.append(f'<line x1="578" y1="{326+r_*26}" x2="648" y2="{326+r_*26}" stroke="#ff8a65" stroke-width="2.5"/>')
    for c in range(4):
        mid.append(f'<rect x="{580+c*19}" y="{320+r_*26}" width="5" height="12" fill="#ff8a65" opacity="0.55"/>')
# android rack (armory)
for i in range(3):
    mid.append(f'<rect x="{656+i*9}" y="348" width="7" height="22" rx="3" fill="#241a20" stroke="#e040fb" stroke-width="1.2"/>')
    mid.append(f'<circle cx="{659.5+i*9}" cy="353" r="1.6" fill="#e040fb"/>')
mid.append('<text x="678" y="379" fill="#e040fb" font-size="7" text-anchor="end" font-weight="700">ANDROID RACK</text>')
# Med bay
...
# ================================================================ WALL CLASSES (destructibility layer)
walls = []
# Class A — partition walls (breachable): fill inter-room gaps, crack glyph
part_gaps = [(430, 288, 94), (556, 288, 94), (682, 288, 94),          # port row
             (470, 418, 94), (576, 418, 94), (642, 418, 94), (722, 418, 94)]  # stbd row
for (gx, gy, gh) in part_gaps:
    walls.append(f'<rect x="{gx}" y="{gy}" width="8" height="{gh}" fill="#39434e"/>')
    cxm = gx + 4; cym = gy + gh / 2
    walls.append(f'<polyline points="{cxm-2},{cym-10} {cxm+2},{cym-4} {cxm-2},{cym+2} {cxm+2},{cym+10}" fill="none" stroke="#8b949e" stroke-width="1.2"/>')
# Class A — fore partitions (officer/CIC, CIC/comms)
for py in (327, 465):
    walls.append(f'<rect x="822" y="{py}" width="103" height="8" fill="#39434e"/>')
    walls.append(f'<polyline points="848,{py+2} 858,{py+6} 868,{py+2} 878,{py+6}" fill="none" stroke="#8b949e" stroke-width="1.2"/>')
# Class B — structural bulkheads (sealed): thick riveted bands
for (bx, by, bh) in [(117, 296, 208), (349, 268, 264), (812, 268, 264)]:
    walls.append(f'<rect x="{bx}" y="{by}" width="7" height="{bh}" fill="#2c3947" stroke="#5c7186" stroke-width="1"/>')
    for ry in range(by + 10, by + bh - 4, 22):
        walls.append(f'<circle cx="{bx+3.5}" cy="{ry}" r="1.4" fill="#7d8fa3"/>')
# Class C — hull depressurization-risk markers (warning triangles along hull-adjacent edges)
for tx in (450, 545, 640, 735):
    walls.append(f'<path d="M{tx} 279 l4 -7 l4 7 Z" fill="none" stroke="#ffd54f" stroke-width="1.2"/>')
for tx in (420, 520, 615, 700):
    walls.append(f'<path d="M{tx} 521 l4 7 l4 -7 Z" fill="none" stroke="#ffd54f" stroke-width="1.2"/>')
add('<g id="wall-classes">' + "".join(walls) + '</g>')
...
doors.append('<text x="357" y="440" fill="#ffd54f" font-size="8" text-anchor="middle">BLAST</text>')
doors.append('<text x="812" y="440" fill="#ffd54f" font-size="8" text-anchor="middle">BLAST</text>')
# connector -> CIC door
doors.append(door(818, 392, 8, 16))
add('<g id="doors" filter="url(#glow)">' + "".join(doors) + '</g>')

# section bulkhead lines on hull (replaced by Class B bands — kept only as faint section ticks)
add('<line x1="120" y1="296" x2="120" y2="504" stroke="#5c7186" stroke-width="2" opacity="0.25"/>')
add('<line x1="352" y1="270" x2="352" y2="530" stroke="#5c7186" stroke-width="2" opacity="0.25"/>')
add('<line x1="816" y1="270" x2="816" y2="530" stroke="#5c7186" stroke-width="2" opacity="0.25"/>')

# section labels along top of hull
add("""
<g id="section-labels" fill="#7d8fa3" font-size="10" letter-spacing="2" text-anchor="middle">
  ...
  <text x="990" y="286" transform="rotate(9 990 286)">— FORE / COMMAND —</text>
</g>
""")

# ================================================================ HULL BREACH / BOARDING MARKERS
br = []
# port breach
...
obj.append('<text x="222" y="562" fill="#ff8a80" font-size="11.5" text-anchor="middle" font-weight="700">&#9680; PRIMARY TARGET — REACTOR</text>')
obj.append('<circle cx="1020" cy="400" r="48" fill="none" stroke="#ff5252" stroke-width="2" stroke-dasharray="8 6" opacity="0.9"/>')
obj.append('<text x="1015" y="338" fill="#ff8a80" font-size="11" text-anchor="middle" font-weight="700">SECONDARY — BRIDGE</text>')
...
# ================================================================ LEGEND
leg = []
leg.append('<rect x="40" y="648" width="1120" height="196" fill="#0b1017" stroke="#30363d" stroke-width="1.5" rx="8"/>')
leg.append('<text x="60" y="676" fill="#e6edf3" font-size="15" font-weight="700" letter-spacing="1">LEGEND &amp; MISSION FLOW</text>')

def swatch(x, y, fill, stroke, label, dy=4):
    return (f'<rect x="{x}" y="{y}" width="15" height="15" fill="{fill}" stroke="{stroke}" stroke-width="1.5"/>'
            f'<text x="{x+23}" y="{y+12}" fill="#c9d1d9" font-size="11.5">{label}</text>')

col1 = [
    ("#1a237e", "#7986cb", "Command / CIC"),
    ("#14213d", "#5c7cfa", "Comms / Sensors"),
    ("#311b1b", "#ef5350", "Engineering / Engines"),
    ("#0f2d23", "#69f0ae", "Airlock / Entry"),
]
col2 = [
    ("#2a1f3d", "#ce93d8", "Crew Quarters"),
    ("#1f3038", "#80cbc4", "Medical"),
    ("#1a2f3a", "#4dd0e1", "Life Support"),
    ("#1f2e1f", "#81c784", "Hydroponics"),
]
col3 = [
    ("#2e2a1f", "#ffb74d", "Cargo / Storage"),
    ("#3e2723", "#ff8a65", "Armory / Security"),
    ("#33261d", "#a1887f", "Mess / Crew Ops"),
    ("#141a22", "#8b949e", "Corridors / Spine"),
]
for i, (f, s, l) in enumerate(col1):
    leg.append(swatch(60, 690 + i*24, f, s, l))
for i, (f, s, l) in enumerate(col2):
    leg.append(swatch(260, 690 + i*24, f, s, l))
for i, (f, s, l) in enumerate(col3):
    leg.append(swatch(460, 690 + i*24, f, s, l))

# markers column
leg.append('<rect x="660" y="690" width="15" height="8" fill="#ffd54f" stroke="#8d6e00" stroke-width="1"/>')
leg.append('<text x="683" y="698" fill="#c9d1d9" font-size="11.5">Door / Blast door</text>')
leg.append('<rect x="660" y="714" width="15" height="10" fill="url(#hazard)" stroke="#ffd54f" stroke-width="1"/>')
leg.append('<text x="683" y="723" fill="#c9d1d9" font-size="11.5">Hull breach (entry)</text>')
leg.append('<circle cx="667" cy="744" r="8" fill="none" stroke="#ff5252" stroke-width="2" stroke-dasharray="4 3"/>')
leg.append('<text x="683" y="748" fill="#c9d1d9" font-size="11.5">Mission objective</text>')
leg.append('<polygon points="667,766 660,774 674,774" fill="#69f0ae"/>')
leg.append('<text x="683" y="772" fill="#c9d1d9" font-size="11.5">Insertion vector</text>')

# mission flow text
leg.append("""
<text x="880" y="692" fill="#e6edf3" font-size="12.5" font-weight="700">MISSION LOOP</text>
<text x="880" y="712" fill="#9fb0c1" font-size="11.5">1  BOARD — breach hull at airlock</text>
<text x="880" y="730" fill="#9fb0c1" font-size="11.5">2  FIGHT — push the spine, clear flanking rooms</text>
<text x="880" y="748" fill="#9fb0c1" font-size="11.5">3  DESTROY — charge on reactor (or bridge)</text>
<text x="880" y="766" fill="#9fb0c1" font-size="11.5">4  EXTRACT — fall back to breach before blowout</text>
""")
# wall-class strip
leg.append('<rect x="60" y="788" width="16" height="14" fill="#39434e"/>')
leg.append('<polyline points="66,790 70,795 66,800" fill="none" stroke="#8b949e" stroke-width="1.2"/>')
leg.append('<text x="84" y="799" fill="#c9d1d9" font-size="11.5">A — Partition: breachable (HP 300, chips under fire)</text>')
leg.append('<rect x="440" y="788" width="8" height="14" fill="#2c3947" stroke="#5c7186" stroke-width="1"/>')
leg.append('<circle cx="444" cy="792" r="1.2" fill="#7d8fa3"/><circle cx="444" cy="798" r="1.2" fill="#7d8fa3"/>')
leg.append('<text x="456" y="799" fill="#c9d1d9" font-size="11.5">B — Structural bulkhead: sealed, indestructible</text>')
leg.append('<path d="M800 801 l5 -8 l5 8 Z" fill="none" stroke="#ffd54f" stroke-width="1.4"/>')
leg.append('<text x="814" y="799" fill="#c9d1d9" font-size="11.5">C — Outer hull: explosion = depressurization risk</text>')

# android rack row
leg.append('<rect x="60" y="810" width="7" height="16" rx="3" fill="#241a20" stroke="#e040fb" stroke-width="1.2"/>')
leg.append('<rect x="69" y="810" width="7" height="16" rx="3" fill="#241a20" stroke="#e040fb" stroke-width="1.2"/>')
leg.append('<text x="84" y="822" fill="#c9d1d9" font-size="11.5">Android rack — vacuum-rated breach response (hunt, then seal); destroy racks in advance to unlock venting tactics</text>')

# procgen note
leg.append('<text x="60" y="838" fill="#7d8fa3" font-size="10.5">PROCGEN NOTE — rooms = nodes, doors = edges, walls = typed edges (A/B/C), spine = critical path. Gated sections are always Class-B bounded: breaching changes how you move, never what you must do.</text>')
...
svg = "\n".join(S)
out = "/mnt/agents/output/assets/mockups/cruiser_boarding_deck_plan.svg"
